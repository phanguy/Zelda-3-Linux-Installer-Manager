...
gui_checklist() {
    ...
    if [ "$GUI_TOOL" = "kdialog" ]; then
        ...
        local res
        res=$(zenity --list --checklist --width=900 --height=800 --title="$title" --text="$formatted_text" --no-markup             --ok-label="$ok_btn" --cancel-label="$cancel_btn"             --column="Select" --column="ID" --column="Description" --hide-column=2 --print-column=2             "${zenity_args[@]}" 2>/dev/null)
        local status=$?
        if [ $status -ne 0 ]; then
            return $status
        fi
...
gui_radiolist() {
    ...
    if [ "$GUI_TOOL" = "kdialog" ]; then
        ...
        zenity --list --radiolist --width=720 --height=560 --title="$title" --text="$formatted_text" --no-markup             --ok-label="$ok_btn" --cancel-label="$cancel_btn"             --column="Select" --column="ID" --column="Description" --hide-column=2 --print-column=2             "${zenity_args[@]}" 2>/dev/null
    fi
}
...
gui_getopenfilename() {
    ...
    if [ "$GUI_TOOL" = "kdialog" ]; then
        ...
        if ! zenity --info --width=450 --title="Action Required"             --text="$title

Click OK to browse and select the file." --no-markup             --ok-label="OK" 2>/dev/null; then
            return 1
        fi
...
Click OK to browse and select the folder." --no-markup             --ok-label="OK" 2>/dev/null; then
            return 1
        fi
...
gui_menu() {
    ...
    if [ "$GUI_TOOL" = "kdialog" ]; then
        ...
        zenity --list --width=720 --height=560 --title="$title" --text="$formatted_text" --no-markup             --ok-label="$ok_btn" --cancel-label="$cancel_btn"             --column="ID" --column="Action" --hide-column=1 --print-column=ALL             "${zenity_args[@]}" 2>/dev/null
    fi
}
...
get_ini() {
    ...
    local val
    val=$(grep -iE "^[# ]*${key} *=" "$file" | head -n1 | sed -E "s|^[# ]*${key} *= *||I" | tr -d '' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
    if [ -n "$val" ]; then
        echo "$val"
    else
        echo "$default_val"
...
configure_bugfixes() {
    ...
    while true; do
        ...
        local choice
        choice=$(gui_menu "Select an option below to view its explanation and change its value:" "Bug Fixes & Exploits"             "misc"  "🐛 MiscBugFixes: Engine, Boss & Audio/Visual Fixes (Currently: $misc_status)"             "game"  "⚡ GameChangingBugFixes: Sequence Breaks & Wall Clips (Currently: $game_status)"             "view"  "📋 View All Bug Fixes & Exploits Explained"             "done"  "💾 Save & Return to Categories")
...
Click OK to change this setting, or Cancel to go back." "Configure: Misc Bug Fixes"; then
            ...
            local new_action
            new_action=$(gui_radiolist "Select setting for MiscBugFixes:" "Configure: Misc Bug Fixes"                 "1" "ENABLE MiscBugFixes (Recommended - Polished experience)" "$def_1"                 "0" "DISABLE MiscBugFixes (Keep original 1991 bugs)" "$def_2")
...
Click OK to change this setting, or Cancel to go back." "Configure: Game-Changing Bug Fixes"; then
            ...
            local new_action
            new_action=$(gui_radiolist "Select setting for GameChangingBugFixes:" "Configure: Game-Changing Bug Fixes"                 "0" "DISABLE Fixes (Recommended for Speedruns - Keep Classic Glitches)" "$def_2"                 "1" "ENABLE Fixes (Enforce strict collision & patch wall-clipping)" "$def_1")
...
configure_display() {
    ...
    while true; do
        ...
        local choice
        choice=$(gui_menu "Select a display option below to modify:" "Display & Graphics Settings"             "aspect"     "📐 Aspect Ratio (Current: $cur_ar)"             "fullscreen" "🖥️ Screen Mode: Toggle Fullscreen / Windowed (Current: $fs_str)"             "scale"      "🔍 Window Scale: 1x, 2x, 3x, 4x (Current: ${cur_scale}x)"             "filter"     "🎨 Pixel Filter: Toggle Crisp / Smooth (Current: $filt_str)"             "flashes"    "⚡ Dim Flashes: Toggle Photosensitivity Guard (Current: $flashes_str)"             "done"       "💾 Save & Return to Categories")
        ...
        if [[ "$choice" =~ "aspect" ]]; then
            ...
            local sel_ar
            sel_ar=$(gui_radiolist "Select your preferred display aspect ratio:" "Aspect Ratio"                 "16:10" "16:10 (Steam Deck Native - Full screen without borders)" "$ar_16_10_on"                 "16:9" "16:9 (Standard Widescreen PC Monitors & Modern TVs)" "$ar_16_9_on"                 "4:3" "4:3 (Original 1991 Retro SNES ratio with pillarboxes)" "$ar_4_3_on"                 "18:9" "18:9 (Ultra-wide 2:1 displays)" "$ar_18_9_on")
...
Saved to zelda3.ini." "Updated"
            ...
            local sel_scale
            sel_scale=$(gui_radiolist "Select window scale multiplier (used in Windowed mode):" "Window Scale"                 "1" "1x (256x224 - Original SNES resolution)" "$sc_1"                 "2" "2x (512x448)" "$sc_2"                 "3" "3x (768x672 - Recommended for 1080p)" "$sc_3"                 "4" "4x (1024x896 - Recommended for 1440p / 4K)" "$sc_4")
...
configure_gameplay() {
    ...
    while true; do
        ...
        local choice
        choice=$(gui_menu "Select an option below to customize features or quickly toggle all:" "Gameplay & QoL Tweaks"             "checklist"    "✏️ Customize Features (Checklist: $count of 12 Enabled)"             "select_all"   "✅ Select All (Enable all 12 QoL features)"             "deselect_all" "❌ Deselect All (Disable all features / Vanilla 1991)"             "defaults"     "⭐ Recommended Defaults (Standard QoL Pack: 9 of 12)"             "done"         "💾 Save & Return to Categories")
...
(Checked items will be saved as ENABLED in zelda3.ini)" "Gameplay & QoL Tweaks"                 "ItemSwitchLR" "Quick item swap (L/R) & Reorder Inventory (Y + Arrows)" "$(s_on "$c_ItemSwitchLR")"                 "TurnWhileDashing" "Turn corners while dashing with Pegasus Boots" "$(s_on "$c_TurnWhileDashing")"                 "BreakPotsWithSword" "Break ceramic pots with Master Sword" "$(s_on "$c_BreakPotsWithSword")"                 "CollectItemsWithSword" "Pick up hearts, rupees, and bombs with sword slash" "$(s_on "$c_CollectItemsWithSword")"                 "DisableLowHealthBeep" "Mute the constant low-health alarm beep" "$(s_on "$c_DisableLowHealthBeep")"                 "CancelBirdTravel" "Cancel bird flute flight immediately with X button" "$(s_on "$c_CancelBirdTravel")"                 "ShowMaxItemsInYellow" "Highlight maxed bombs, arrows, and rupees in yellow text" "$(s_on "$c_ShowMaxItemsInYellow")"                 "SkipIntroOnKeypress" "Skip opening intro sequence by pressing any button" "$(s_on "$c_SkipIntroOnKeypress")"                 "MoreActiveBombs" "Allow up to 4 active bombs at once instead of 2" "$(s_on "$c_MoreActiveBombs")"                 "EnhancedMode7" "Higher quality world map and perspective 3D rendering" "$(s_on "$c_EnhancedMode7")"                 "MirrorToDarkworld" "Allow Magic Mirror to warp back TO the Dark World" "$(s_on "$c_MirrorToDarkworld")"                 "CarryMoreRupees" "Increase maximum wallet capacity to 9999 rupees" "$(s_on "$c_CarryMoreRupees")")
            local cl_status=$?
...
configure_audio() {
    ...
    while true; do
        ...
        local choice
        choice=$(gui_menu "Select an audio option below to modify:" "Audio Settings"             "toggle_msu"  "🎵 Toggle MSU-1 Audio (Currently: $msu_str)"             "install_msu" "📦 Install / Extract an MSU-1 Audio Pack (.zip, .7z, or folder)"             "freq"        "🔊 Sample Rate: 44100 Hz / 48000 Hz (Current: ${cur_freq} Hz)"             "channels"    "🎧 Channels: Toggle Stereo / Mono (Current: $chan_str)"             "done"        "💾 Save & Return to Categories")
...
Saved to zelda3.ini." "Audio Updated"
            ...
            local msu_type
            msu_type=$(gui_radiolist "What format is your MSU-1 pack in?" "MSU Source Type"                 "archive" "A compressed archive file (.zip, .7z, .rar, .tar, .gz)" on                 "folder" "An uncompressed folder containing the .pcm files" off)
...
$game_dir/msu/" "MSU Complete"
            ...
            local sel_freq
            sel_freq=$(gui_radiolist "Select audio output frequency:" "Audio Frequency"                 "44100" "44100 Hz (Standard CD Quality - Recommended)" "$([ "$cur_freq" == "44100" ] && echo "on" || echo "off")"                 "48000" "48000 Hz (Studio / Broadcast Quality)" "$([ "$cur_freq" == "48000" ] && echo "on" || echo "off")")
...
• Gameplay QoL: $qol_count of 12 features enabled"
        ...
        local cat_choice
        cat_choice=$(gui_menu "Select a category below to tweak, or select 'Save & Exit':" "Zelda 3 Settings & Tweaks"             "display"   "📺 Display & Graphics ($cur_ar, $fs_str, $filt_str)"             "gameplay"  "🎮 Gameplay & Quality of Life ($qol_count of 12 Enabled)"             "bugfixes"  "🐛 Bug Fixes & Exploits (Misc: $misc_str | Sequence Breaks: $game_str)"             "audio"     "🔊 Audio & MSU-1 Setup (MSU-1: $msu_str | ${cur_freq}Hz)"             "status"    "📋 View Live zelda3.ini Status Summary"             "done"      "💾 Save & Exit to Manager Hub")
...
Select an option below:" "Zelda 3 Manager Hub"             "settings"  "⚙️ Configure Game Settings & Tweaks"             "backup"    "💾 Backup Save Data"             "restore"   "📂 Restore Save Data"             "reinstall" "🔄 Reinstall / Repair Game"             "exit"      "🚪 Exit Manager")
